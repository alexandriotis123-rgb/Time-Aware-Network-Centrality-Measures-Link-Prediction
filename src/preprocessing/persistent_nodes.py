...
def restrict_graph( graph, persistent_nodes):
    """
    ...
    restricted_graph = graph.subgraph(persistent_nodes).copy()
...
def build_persistent_pairs(subgraphs):
    ...
    for i in range(len(subgraphs) - 1):
        ...
        persistent_nodes = find_persistent_nodes(graph_1, graph_2)

        persistent_node_sets.append(persistent_nodes)

        restricted_graph_1 = restrict_graph(graph_1,persistent_nodes)

        restricted_graph_2 = restrict_graph(graph_2, persistent_nodes)

        persistent_pairs.append((restricted_graph_1, restricted_graph_2))
